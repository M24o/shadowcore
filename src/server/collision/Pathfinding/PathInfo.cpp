...
////////////////// PathInfo //////////////////
PathInfo::PathInfo(const WorldObject* from, const float destX, const float destY, const float destZ, bool useStraightPath) : m_pathPolyRefs(NULL), 
    m_polyLength(0), m_pointPathPointer(0), m_type(PATHFIND_BLANK), m_useStraightPath(useStraightPath), m_sourceObject(from), 
...
void PathInfo::Update(const float destX, const float destY, const float destZ, bool useStraightPath)
{
    ...
    float dist = 2*m_sourceObject->GetObjectSize();
...
void PathInfo::BuildPointPath(float *startPoint, float *endPoint)
{
    ...
    float closestPoint[VERTEX_SIZE];
    if((m_type & PATHFIND_INCOMPLETE) && m_navMeshQuery->closestPointOnPoly(m_pathPolyRefs[m_polyLength-1], endPoint, closestPoint))
    {
        dtVcopy(endPoint, closestPoint);
        setActualEndPosition(PathNode(endPoint[2],endPoint[0],endPoint[1]));
    }
...
bool PathInfo::getSteerTarget(const float* startPos, const float* endPos, const float minTargetDist, const dtPolyRef* path, 
    const uint32 pathSize, float* steerPos, unsigned char& steerPosFlag, dtPolyRef& steerPosRef, float* outPoints, uint32* outPointCount)
{
    // Find steer target.
    ...
    uint32 nsteerPath = m_navMeshQuery->findStraightPath(startPos, endPos, path, pathSize, steerPath, steerPathFlags, steerPathPolys, MAX_STEER_POINTS);
    if (!nsteerPath)
        return false;
...
uint32 PathInfo::findSmoothPath(const float* startPos, const float* endPos, const dtPolyRef* path, const uint32 pathSize, float* smoothPath, 
    const uint32 maxSmoothPathSize)
{
